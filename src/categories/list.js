  ...
  render() {
    return (
        <div className="db tc ma2 mt4">
            {this.state.names.map( (c,i) => <Link key={c.path} className={`f6 ph3 pv2 mb2 dib white bg-purple mh2 no-underline dim pointer animated flipInY anim-del-0${i}0 `} to={`/${c.name}`} >{ c.name }</Link>)}
        </div>
    );
  }
}
